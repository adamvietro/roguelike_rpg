...
impl State {
    ...
    /// them. Video's Fullscreen row is real again as of 2026-09-16 (see
    /// FullscreenSetting in settings.rs) - it was briefly paused
    /// 2026-09-15 after going fullscreen exposed a separate, real,
    ...
    /// APP on every resize, and `battle_backgrounds.png` used to be
    /// registered at a full 1280x800 "tile size" for a single giant
    /// glyph - see BATTLE_BACKDROP_CONSOLE's own doc comment in main.rs
    /// for the full incident and the fix). `BTermBuilder::with_
    /// fullscreen` is only ever read once at window-creation time in
    /// `main()`, not live - so toggling it here changes next launch, not
    /// this session, and the row says so. Video's OTHER row, Window
