...
#[read_component(ProvidesInvisibility)]
    ...
    <(Entity, &ActivateItem)>::query()
        ...
        .for_each(|(entity, activate)| {
            ...
            if let Ok(item) = item {
                ...
                // Invisible Cloak - grants Invisible for `moves` turns. Using
                // another cloak while already Invisible just overwrites it
                // with a fresh count, matching how other refreshable timed
                // effects in this project already behave (see battle::DotState).
                if let Ok(cloak) = item.get_component::<ProvidesInvisibility>() {
                    commands.add_component(
                        activate.used_by,
                        Invisible {
                            moves_remaining: cloak.moves,
                        },
                    );
                }
            }
